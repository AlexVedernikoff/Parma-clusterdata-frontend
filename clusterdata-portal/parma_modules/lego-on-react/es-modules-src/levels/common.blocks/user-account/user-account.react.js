...
export default decl({
    ...
    content: function content(_ref4) {
        ...
        if (pic) {
            ...
            content.push(React.createElement(UserAccountPic, _extends({ key: 'pic' }, { pic: pic, avatarId: avatarId, avatarHost: avatarHost, provider: provider, plus: plus })));
        }
        ...
        if (name !== false) {
            if (subname && subname !== name) {
                ...
                content.push(React.createElement(UserAccountName, _extends({ key: 'name' }, { hasAccentLetter: hasAccentLetter, text: name })));
            }
        }

        if (this.context.processLogout && !isCurrent) {
            return content.concat(React.createElement(Icon, {
                key: 'logout',
                glyph: 'x-sign',
                size: 'xs',
    ...
    _changeUser: function _changeUser(e) {
        ...
        !this.props.isCurrent && !this.props.isAddAccount && this.context.changeUser && this.context.changeUser(e, this.props.uid);
    },
