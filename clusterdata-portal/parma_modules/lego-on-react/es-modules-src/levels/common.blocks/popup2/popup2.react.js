...
export var AVAILABLE_DIRECTIONS = ['bottom-left', 'bottom-center', 'bottom-right', 'top-left', 'top-center', 'top-right', 'right-top', 'right-center', 'right-bottom', 'left-top', 'left-center', 'left-bottom'];
...
export default decl({
    ...
    willInit: function willInit() {
        ...
        /*%%%ISLDEBUG%%%*/+0 && console.assert(typeof this.props.scope === 'function', 'Передача ссылки на элемент в scope является устаревшим подходом ' + 'и в следующей мажорной версии перестанет работать. ' + 'В scope необходимо передать функцию, которая должна возвращать ссылку на элемент.');

        /*%%%ISLDEBUG%%%*/+0 && console.assert(typeof this.props.onOutsideClick !== 'undefined', 'onOutsideClick является устаревшим свойством ' + 'и в следующей мажорной версии перестанет работать. ' + 'Необходимо использовать свойство onClose.');
    ...
    render: function render() {
        ...
        return createPortal(React.createElement(
            ...
            hasTail && React.createElement(PopupTail, {
    ...
    _calcTailPos: function _calcTailPos(direction, target, popup, pos) {
        ...
        if (this._checkSecondaryDirection(direction, 'right')) {
            ...
            res.top = popup.popupHeight - Math.ceil(Math.min(popup.popupHeight, target.height) / 2) - halfOfTail - tailOffset;
        } else if (this._checkSecondaryDirection(direction, 'top')) {
    ...
    _calcOffsets: function _calcOffsets() {
        ...
        return {
            main: mainOffset >= 0 && tailDimension > 0 ? Math.max(mainOffset, Math.round(tailDimension * Math.SQRT1_2)) : mainOffset,
