...
export default decl({
    ...
    content: function content(_ref4) {
        ...
        val = val === undefined ? [] : [].concat(val).filter(function (val) {
        ...
        if (text === undefined && width !== 'fixed') {
            /*%%%ISLDEBUG%%%*/+0 && console.error('В следующей мажорной версии значение vary для поля text ' + 'перестанет проставляться по умолчанию, пожалуйста, ' + 'при необходимости передавайте значение явно.');

            text = 'vary';
        }

        placeholder = text !== 'vary' || !val.length ? placeholder : flattenItems.reduce(function (res, item) {
        ...
        return [React.createElement(SelectButton, _extends({ key: 'button' }, {
            val: val,
            tone: tone,
            view: view,
            opened: opened,
            ...
            items: flattenItems,
            multiple: type !== 'radio'
        }), React.createElement(SelectPopup, _extends({ key: 'popup' }, {
            maxHeight: maxHeight,
            items: children,
