...
function isWizard(type) { return /_wizard$/.test(type); }
...
function graphToTable(data, config) {
    ...
    data.graphs.forEach((graph) => head.push({
...
function postRun(loaded) {
    ...
    const denormalizedParams = Object.keys(params)
        .reduce((result, key) => {
...
class Charts {
    static async load({id, source, params, editMode: {config, type} = {}, paginateInfo, headers, cancelToken, orderBy}) {
        const isEditMode = config && type;
        try {
            const {data} = await axiosInstance(settings.requestDecorator({
