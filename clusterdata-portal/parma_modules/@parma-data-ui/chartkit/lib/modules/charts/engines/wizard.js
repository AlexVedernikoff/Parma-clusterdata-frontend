        ...
        graph.plotOptions = {
            series: {
                dataLabels: {
                    ...
                    formatter: function () {
                        if ((config.showValues === 'onlyLast' && this.point.index === this.series.data.length - 1)
...
function _replaceWizardParam(target, parameterization) {
    return target ?
        target.replace(/\$([A-Za-z0-9_]+)/g,
            (str, param) => parameterization[param] ? parameterization[param][0] : param) :
        target;
}

// TODO: разобраться с parameterization и params
function _processConfig({axis, config, highstock, legend, order, tooltip, parameterization = {}}, {categories_ms}, params, configName) {
    ...
    if (config.comments && config.comments === 'yes') {
        statface_graph.comments = {
            path: configName,
            matchedParams: Object.keys(parameterization).map((key) => parameterization[key]).reduce((result, {name, match}) => {
                if (match) {
                    result.push(name);
                }
                return result;
    ...
    try {
        ...
        switch (type) {
            case CONFIG_TYPE.GRAPH_WIZARD:
                ...
                result.comments = comments ? prepareComments(comments, data.graphs.map(({id}) => id)) : null;
        ...
        throw ErrorDispatcher.wrap(
            {type: ERROR_TYPE.EXECUTION_ERROR, extra: {tabName: 'wizard', executionError: error}}
        );
    }
}
