...
// TODO: нужен отдельный класс
// TODO: вынести константу с типами (есть еще у формы модального окна комментариев)
...
function extendComment({meta, ...rest}) {
...
function isNotFitGrid(x, chart) {
    const {userOptions: {_config: {comments: {ignoreScale} = {}} = {}}} = chart;
    const isXonXAxis = chart.xAxis[0].series.some(({xData}) => xData.indexOf(x) !== -1);
    return !isXonXAxis && !ignoreScale && x % chart.xAxis[0].closestPointRange !== 0;
}

function isGridNotFitBetween(from, to, chart) {
    const {userOptions: {_config: {comments: {ignoreScale} = {}} = {}}} = chart;
...
function drawFlagX(chart, {x, title, text, y, shape, color = DEFAULT_COLOR}) {
...
function drawLineX(chart, {x, color = DEFAULT_COLOR, width, dashStyle, text}) {
    if (isNotFitGrid(x, chart)) {
        return;
    }
    const line = chart.xAxis[0].addPlotLine({
        ...
        id: Math.random().toFixed(3).toString(), // TODO: нужно ли это?
        zIndex: Z_INDEX
    });
...
function drawBandX(chart, options, onlyRenderer) {
    if (options.x && isNotFitGrid(options.x, chart)
    ...
    const closesPointRange = options.from && options.to && options.from !== options.to ?
    ...
    if (yFromPx) {
        chart.renderer.rect(xFromPx, yFromPx, width, 4, 0)
            .attr({
...
function getDotLabelPositions(options) {
    ...
    if (pointY > heightPlusMargin) {
        ...
        anchor = ['M', pointX, pointY - DOT_HALF_MARGIN,
            'L', pointX - DOT_RADIUS, pointY - DOT_MARGIN, pointX + DOT_RADIUS, pointY - DOT_MARGIN,
            'Z'];
    ...
    if (pointX + widthPlusMargin < plotRight) {
        x = pointX + DOT_MARGIN;
        anchor = ['M', pointX + DOT_HALF_MARGIN, pointY,
            'L', pointX + DOT_MARGIN, pointY + DOT_RADIUS, pointX + DOT_MARGIN, pointY - DOT_RADIUS,
            'Z'];
...
function drawDotLabel(chart, options) {
    ...
    const label = chart.renderer.label(options.text, pointX, pointY, 'callout')
        .css({
...
function translateDotsLabels(chart, notTranslatedDotsLabels) {
    ...
    notTranslatedDotsLabels.sort((a, b) => { return a.label.x - b.label.x; })
        .forEach((label, index) => {

            // массив доступных для размещения label позиций
            const available = label.positions.map((position) => {
                return {
                    ...
                    previous: translatedDotsLabels.filter((translated) => {
                        return isRectsIntersects(
                            ...
                            translated.x, translated.x + translated.w,
                            translated.y, translated.y + translated.h
                        );
                    }).length,
                    ...
                    next: notTranslatedDotsLabels
                        ...
                        .map((notTranslated) => {
                            ...
                            return notTranslated.positions.filter((notPosition) => {
                                return isRectsIntersects(
                                    notPosition.x, notPosition.x + notPosition.w,
                                    notPosition.y, notPosition.y + notPosition.h,
                                    position.x, position.x + position.w,
                                    position.y, position.y + position.h
                                );
                            }).length;
            ...
            chart.renderer.path(available[0].position.anchor)
                .attr({
                    strokeWidth: 1,
...
function drawComments(chart, comments, settings, force) {
    if (force) {
...
function drawOnlyRendererComments(chart, comments = [], settings) {
...
function hideComments(chart, comments, settings, force) {
    ...
    if (chart && chart.container && comments) {
        ...
        comments.forEach((comment) => {
            ...
            if (extendedComment.type === TYPES.DOT_XY) {
                ...
                if (seriesIndex !== -1) {
                    const point = chart.series[seriesIndex].data.find((_point) => _point &&
                        _point.x ===
                        extendedComment.x);
                    if (point) {
...
function getCommentsOnLine(xLine, comments) {
    const xComments = [];
    const xyComments = [];

    if (comments) {
        comments.forEach((_comment) => {
            const comment = extendComment(_comment);
            ...
            } else if (comment.from && comment.from <= xLine.x && xLine.x <= comment.to) {
...
//     Chart.prototype.shouldDrawComments = function () {
//         return this.chart && this.chart.series.some(function (graph) {
//             return graph.visible && graph.name !== 'legend_manage_line';
//         }) && !this.settings.hideComments;
//     };
//
//     Chart.prototype.anyComments = function () {
//         return this.data && this.data.comments;
//     };
// }
