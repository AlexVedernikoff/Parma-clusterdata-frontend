...
ChartMetric.prototype.processConfig = function (data) {
    ...
    if (content) {
        ...
        if (content.diffPercent && (content.diffPercent.formatted || content.diffPercent.value == undefined)) {
            if (content.diffPercent.value == undefined && (content.last.value == undefined || content.current.value == undefined)) {
                delete content.diffPercent;
            } else {
                diff = (Number(content.current.value) - Number(content.last.value)) / Number(content.last.value);
                formattedValue = this.formatValue((content.diffPercent.value == undefined) ? diff : content.diffPercent.value, true, 'percentage');
...
ChartMetric.prototype.formatValue = function (value, withSign, format) {
    ...
    switch (format) {
        case 'percentage':
            ...
            break;
        default:
            while (Math.round(value) >= 999.5) { // 999.5 because greater values will be rounded to 1000
                value = value / 1000;
                rank++;
            }
            unit = suffixes[rank];
...
ChartMetric.prototype.getBackground = function (data, green, red) {
    ...
    if (content.diffPercent) {
        ...
        diffContent = this.formatValue((Number(content.current.value) - Number(content.last.value)) / Number(content.last.value), true, 'percentage');
    }
