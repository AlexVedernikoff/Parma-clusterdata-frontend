...
${
    data.onlyDate ?
        `<div class="_tooltip-date">
            ...
            ${
                data.dayOfWeek ?
                    `<span class="_tooltip-date-dayofweek${data.holiday || data.weekend ? ' _tooltip-date-dayofweek_weekend' : ''}">
                        ${data.dayOfWeek}
                    </span>` :
                    ''
            }
                    ...
                    ${data.lines.map((line) =>
                        `<tr>
                            <td class="_tooltip-rows__buble-td">
                                <div class="_tooltip-rows__buble-div" style="background-color:${line.seriesColor};"></div>
                            </td>
                            ...
                            line.commentText || line.xyCommentText ?
                                `<tr>
                                    <td class="_tooltip-rows__comment-left-td">
                                        <div class="_tooltip-rows__comment-left-div" style="background:${line.seriesColor};"/>
                                    </td>
                                    <td colspan="10">
                                        ${line.commentText ? `<div class="_tooltip-rows__comment-div">${line.commentText}</div>` : ''}
                                        ${line.xyCommentText ? `<div class="_tooltip-rows__comment-div">${line.xyCommentText}</div>` : ''}
                                    </td>
                                </tr>` :
                                ''
                        }`
                        ...
                        ${
                            data.holiday ?
                                `<div class="_tooltip-right__holiday-div">
                                    ...
                                    ${data.holidayText}
                                    ${data.region ? `<span class="_tooltip-right__holiday-region">[${data.region}]</span>` : ''}
                                </div>` :
                                ''
                        }
