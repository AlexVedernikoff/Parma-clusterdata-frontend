...
class ChartKit extends React.Component {
    ...
    static setSettings(newSettings) { settings.set(newSettings); }
    ...
    static getDerivedStateFromProps(
        {id, source: propsSource, params: propsParams, editMode, requestIdPrefix, paginateInfo: paginateInfoFromProps, orderBy: orderByFromProps},
        {prevId, prevSource, prevParams, prevEditMode, forceUpdate, requestCancelSource, paginateInfo: paginateInfoFromState, prevPaginateInfo, orderBy: orderByFromState, prevOrderBy}
    ) {
        ...
        const paginateInfo = prevEditMode !== editMode ? editMode.config.shared.paginateInfo : (paginateInfoFromProps ? paginateInfoFromProps : paginateInfoFromState)
        const orderBy = prevEditMode !== editMode ? editMode.config.shared.orderBy : (orderByFromProps ? orderByFromProps : orderByFromState)
    ...
    componentDidMount() {
         ...
         * + удалить костыль из @parma-data-ui\chartkit\lib\components\Widget\Graph\Graph.js где график рисуется каждый раз с нуля
    ...
    onChange = ({params, state}) => this.setState({
        loading: true, error: false, runtimeParams: {...this.state.runtimeParams, ...params}, ...state
    });
    ...
    displayOnlyWithFilter = () => {
        ...
        return <div className={'chartkit__widget_graph chartkit__widget_need-filter'}>Для отображения данных, пожалуйста, выберите значение в фильтре</div>;
    }

    isDisplayOnlyWithFilterAndFiltersIsEmpty = (isDisplayOnlyWithFilter, params) => {
        return isDisplayOnlyWithFilter && Object.values(params).length !== 0 && !Object.values(params).some(item => item.value.length !== 0);
    }
