  ...
  onChange(data) {
    ...
    if (groupField) {
      this.props.onStateAndParamsChange({
        params: {
          [groupField]: this._convertCategoryName(
            data.point.originalCategory,
            this.props.data.data.categoriesDataTypeName,
          ),
        },
      });
    }
  ...
  getPointClick() {
    return data => {
      ...
      point.series.points.forEach(point => {
        point.update(
          { color: '#B8BEC7', borderColor: undefined, borderWidth: 0, originColor: point.originColor || point.color },
          true,
          false,
        );
      });
      point.update(
        { color: color, borderColor: borderColorByColor(color), borderWidth: 2, originColor: color },
        true,
        false,
      );
    };
