...
export default class DownloadScreenshotModal extends React.PureComponent {
    ...
    constructor(props) {
        ...
        if (storageState) {
            ...
            PARAMETERS.forEach((parameter) => { state[parameter.name] = Boolean(parameter.val); });
    ...
    _sizeTitle = (size) =>
        `${i18n(size.key)} (${size.width}x${size.height})`;
                    ...
                    <ChartsModal.Body>
                        <div className={b()}>
                                ...
                                <div className={b('cell', {column: true})}>
                                    ...
                                    <div className={b('cell')}>
                                        <span className="radio-inputs">
                                            {this._textInput('width')}
                                            x
                                            {this._textInput('height')}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className={b('row', {hint: true})}>
                                {i18n('hint-screenshot', {link:
                                    <Link key="wiki-link" theme="normal" url={API_WIKI} target="_blank">API</Link>
                                })}
                            </div>
                        </div>
                    </ChartsModal.Body>
