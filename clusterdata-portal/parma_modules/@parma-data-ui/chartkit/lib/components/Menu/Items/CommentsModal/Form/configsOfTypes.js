...
const EMPTY_COLOR_STYLE = {
    width: '30px',
    height: '30px',
    backgroundImage: 'linear-gradient(45deg, #ececec 25%, transparent 25%, transparent 75%, #ececec 75%), linear-gradient(45deg, #ececec 25%, transparent 25%, transparent 75%, #ececec 75%)',
    backgroundSize: '10px',
        ...
        return {
            ...
            text: (
                <svg width="128" height="36" opacity="0.7">
                    <path fill={fillColor} d="M 8 0 L 120 0 C 128 0 128 0 128 8 L 128 20 C 128 28 128 28 120 28 L 8 28 C 0 28 0 28 0 20 L 0 8 C 0 0 0 0 8 0"/>
...
const DASH_STYLE = {
    ...
    items: DASH_STYLES.map(({value, strokeDashArray}) => {
        return {
            value,
            icon: <ButtonIcon><SvgLine strokeDashArray={strokeDashArray}/></ButtonIcon>
        };
    })
};
...
const DASH_WIDTH = {
    ...
    items: DASH_WIDTHS.map(({value, strokeWidth}) => {
        return {
            value,
            icon: <ButtonIcon><SvgLine strokeWidth={strokeWidth}/></ButtonIcon>
        };
    })
};
