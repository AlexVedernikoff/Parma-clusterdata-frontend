      ...
      <div className={b('row', { info: true })}>
        <div className={b('cell')}>
          {this.props.isStat
            ...
            : `${i18n('comment-created')}: ${moment(this.state.createdDate).format(INFO_DATE_FORMAT)} <${
                this.state.creatorLogin
              }>`}
        </div>
        {this.state.modifiedDate && (
          <div className={b('cell')}>
            {`${i18n('comment-modified')}: ${moment(this.state.modifiedDate).format(INFO_DATE_FORMAT)} <${
