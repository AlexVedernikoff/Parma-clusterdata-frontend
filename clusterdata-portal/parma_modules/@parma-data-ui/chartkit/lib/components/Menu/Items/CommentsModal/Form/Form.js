...
import DatePicker from '@parma-data-ui/react-components/src/components/DatePicker';
import RangeDatePicker from '@parma-data-ui/react-components/src/components/RangeDatePicker';
...
function paramsToText(params) {
    return params ?
        Object.keys(params).map((key) => params[key].map((value) => `${key}=${value}`).join('\n')).join('\n') :
        '';
}
...
function _getNewState({
    id, currentFeed, currentParams, dateMinMs, dateMaxMs, graphs, scale, onAction,
    date, dateUntil, color, feed, graphId, params,
    ...props
}) {
...
export default class Form extends React.PureComponent {
            ...
            <div className={b('row')}>
                ...
                <div className={b('cell')}>
                    <TextArea
                        ...
                        ref={(component) => { this._refs.text = component; }}
                    />
                </div>
            </div>
        );
    ...
    _renderControls() {
        return CONFIGS_OF_TYPES
            .find(({value}) => value === this.state.type)
            .controls
            .map(({type, name, title, ...control}) => {
                        ...
                        <Select
                            ...
                            onChange={([feed]) => {
                                ...
                                if (!this.state.isParamsEditing) {
                                    this.setState({
                                        paramsText: paramsToText(this.props.feeds.reduce(
                ...
                <div className={b('row')}>
                    ...
                    <div className={b('cell')}>
                        <TextArea
                            ...
                            ref={(component) => { this._refs.paramsText = component; }}
                        />
                    </div>
                </div>
            ) :
    ...
    async _onAction() {
        ...
        try {
            const comment = {
                ...
                dateUntil: !this.props.isStat && TYPES_OF_COMMENTS.BAND_X === this.state.type ?
                    this.state.dateUntil :
                    null,
                text: this.state.text,
                meta: CONFIGS_OF_TYPES
                    .find(({value}) => value === this.state.type)
                    .controls
                    .reduce((result, {name}) => {
                        if (Array.isArray(name)) {
                            name.forEach((item) => { result[item] = this.state[item]; });
                        } else {
    ...
    isDraft() {
        ...
        return origin.text !== this.state.text
            ...
            || CONFIGS_OF_TYPES
                .find(({value}) => value === this.state.type)
                .controls
                .some(({name}) => this.state[name] !== origin[name]);
    }
        ...
        return (
            <div className={b()}>
                <div className={b('header')}>
                    {this._isEdit ? i18n('edit-comment') : i18n('add-comment')}
                </div>
                ...
                <div className={b('footer')}>
                    <Button
                        ...
                        ref={(component) => { this._refs.action = component; }}
                    >
