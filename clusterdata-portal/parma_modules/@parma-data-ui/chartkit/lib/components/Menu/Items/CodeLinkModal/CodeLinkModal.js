...
const MENU_ITEMS = [
    ...
    },
    {
        ...
        icon: 'favicon-16.png',
        configTypes: GRAPH_MAP_CONFIG_TYPES.concat([CONFIG_TYPES.TABLE, CONFIG_TYPES.TABLE_NODE, CONFIG_TYPES.TABLE_WIZARD_NODE])
    },
    {
        val: SERVICES.AT,
...
export default class CodeLinkModal extends React.PureComponent {
    ...
    render() {
        const menuItems = MENU_ITEMS
            .filter((item) => !item.configTypes || item.configTypes.includes(this.props.configType))
            .map((item) => {
                return {val: item.val, title: item.title, icon: item.icon};
            });

        const body = this.state.selected === SERVICES.CHARTS && <Charts url={this.props.url}/>
        ...
        return (
            <ChartsModal element={this.props.element}>
                ...
                />
                <ChartsModal.Section>
                    <div className={b({[this.props.configType]: true})}>
                        {body}
                    </div>
                    <ChartsModal.Footer>
                        <Button
