...
export default function createBody(config) {
    ...
    return class CodeLinkModalBody extends React.PureComponent {
        ...
        constructor(props) {
            ...
            config.checkboxes.forEach((checkbox) => { state[checkbox.key] = checkbox.checked; });
        ...
        _checkboxes = () => (
            <div className={b('row', {common: true, column: true})}>
                {config.checkboxes.map((checkbox, index) => {
                    return (
                        <div
                            className={b('cell', {common: index !== 0})}
                            key={checkbox.key}
                        >
                            <CheckBox
                                theme="normal"
        ...
        render() {
            ...
            config.checkboxes.filter((checkbox) => checkbox.queryParam)
                .forEach((checkbox) => {
            ...
            return (
                <div className={b()}>
                    ...
                    <div className={b('row', {common: true, row: true})}>
                        {config.text}
                    </div>
                </div>
            );
