...
class FormulaAutocompleter {
    constructor({getAutocomplete, getAutocompleteTimeout = 1000}) {
        ...
        this.completer = {
            ...
            insertMatch: (editor, {completer, ...data}) => { // eslint-disable-line no-unused-vars
                const {
                    value,
                    posRow,
                    posColumn
    ...
    autocompleteDispatcher = (event) => {
        if (['insertstring', 'backspace', 'del', 'removewordleft', 'removewordright']
            .includes(event.command.name) &&
    ...
    async performAutocomplete(editor, isManual) {
        ...
        try {
            ...
            const items = await this.getAutocomplete({
                text,
                row,
                column,
                isManual
