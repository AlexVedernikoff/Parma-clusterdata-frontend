...
function defaultWrapper(item, {url, target}) {
    return <Link url={url} target={target} theme="normal">{item}</Link>;
}
...
class MenuGroup extends React.Component {
    ...
    renderTitle() {
        ...
        const item = (
            <div className={b('group-title', {current, disabled})}>{title}</div>
        );
        return (
            ...
            <div className={b('group', {current})}>
                ...
                <div className={b('items', {current})} style={{height: this.getItemsHeight()}}>
                    {Array.isArray(items) && items
                        ...
                        .map(item => (
                            <MenuItem key={item.name} {...item} onClick={onItemClick}/>
                        ))
                    }
                </div>
            </div>
    ...
    const item = (
        <div className={b('item', {disabled})}>
            <div className={b('item-icon')}>
                {Boolean(iconData) && (
                    <Icon data={iconData} className={iconClassName}/>
                )}
            </div>
            ...
            {Boolean(alpha) && (
                <div className={b('alpha')}>{alpha}</div>
            )}
        </div>
    );
...
class ServiceMenu extends React.Component {
    ...
    state = {
        searchText: '',
        currentGroup: this.props.menuData.currentGroup ||
