...
class TimelinePicker extends React.Component {
    ...
    renderRefresh() {
        ...
        const title = (refreshIntervals.find(item => item.value === String(refreshInterval)) ||
            refreshIntervals[0]).title;
        const placeholder = (
                    ...
                    <div className={b('shortcuts')}>
                        <div className={b('datepickers')}>
                            {React.createElement(this.props.picker, {
                                from, to, onFromChange: this.onFromChange, onToChange: this.onToChange
                            })}
                        </div>
