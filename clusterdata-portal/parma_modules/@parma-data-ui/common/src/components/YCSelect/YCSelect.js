...
export default class YCSelect extends React.PureComponent {
    ...
    static getInnerValue(propsValue) {
        ...
        return new Set(Array.isArray(propsValue) ?
            propsValue.filter(Boolean) :
            [propsValue]
        );
    }
    ...
    componentDidUpdate() {
        const {
            controlWidth: controlWidthProps,
            showSearch
        } = this.props;
        const {
            controlWidth: controlWidthState
        } = this.state;
        const controlNode = this.controlRef.current;
    ...
    onMultipleSearchButtonClick = () => {
        ...
        if (innerValue.size === this.state.innerValue.size) {
            innerValue = new Set(
                [...this.state.innerValue]
                    .filter(innerVal => !shownItemsValue.has(innerVal))
            );
        }
    ...
    _onBadgeClick = e => {
        ...
        if (getItems) {
            ...
            state.selectedPopupItems = items
                .filter(item => innerValue.has(item.value));
        }
            ...
            <div className={bControl('tokens')}>
                {
                    placeholderIcon && !titles.length && (
                        <div className={bControl('placeholder-icon')}>
                            {placeholderIcon}
                        </div>
                    )
                }
            ...
            >
                ...
                }
                {
                    !isMultiply && showItemIcon && icon && (
                        <div className={bControl('selected-item-icon')}>
                            {icon}
                        </div>
                    )
                }
    ...
    render() {
        ...
        if (this.state.popupWidth) {
            ...
            searchWidth = this.state.popupWidth > this.state.controlWidth ?
