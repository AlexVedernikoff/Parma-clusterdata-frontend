...
export default function DatePickerFactory(moment, DatePicker) {
    ...
    return class RangeDatePicker extends React.PureComponent {
        ...
        static defaultProps = {
            value: {
                from: moment().add(-1, 'day').toISOString(),
        ...
        componentDidUpdate(prevProps, prevState) {
            ...
            const valueChanged = this.localMoment(this.props.value.from).isValid()
        ...
        reportChange() {
            ...
            const fromChanged = selectedFrom !== propsFrom ||
                selectedFrom && propsFrom && !selectedFrom.isSame(propsFrom);
            const toChanged = selectedTo !== propsTo ||
            ...
            const from = (
                selectedFrom &&
                (this.props.useStringsInCallback && typeof selectedFrom !== 'string') ?
            ...
            const to = (
                selectedTo &&
                (this.props.useStringsInCallback && typeof selectedTo !== 'string') ?
        ...
        renderQuickIntervals() {
            ...
            return (
                <div className={b('last-periods')}>
                    <div className={b('last-periods-title')}>
                        {i18n('interval')}:
                    </div>
                    <div className={b('last-periods-elements')}>
                        {this.props.quickIntervals.map((interval, i) => {
                            ...
                            const validInterval = !this.props.minDate ||
                            ...
                            if (validInterval) {
                                return (
                                    <div
                                        key={i}
                                        className={
                                            b('last-periods-element', {active: i === this.state.selectedQuickInterval})
                                        }
                                        onClick={() => {
                                            this.setState({
        ...
        render() {
            return (
                <div className={b({size: this.props.size})} ref={(ref) => {
                    this.baseElem = ref;
                }}
                >
                    <Button
                    ...
                    >
                        ...
                        <span>
                            <span>
                                ...
                                this.localMoment(this.props.value.from).format(this.displayDateFormat)
