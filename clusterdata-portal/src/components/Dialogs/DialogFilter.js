...
class DialogFilter extends PureComponent {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        const isNumber = item.cast === 'integer' || item.cast === 'uinteger' || item.cast === 'float' || item.cast === 'double' || item.cast === 'long';
        ...
        let operation = filter ? availableOperations.find((operation) => {
        ...
        if (item.type === 'DIMENSION' || isDate) {
            ...
            if (isDate) {
                ...
                sdk.bi.getResultBySQL(minRequestParams)
                    .then((data) => {
                ...
                sdk.bi.getResultBySQL(maxRequestParams)
                    .then((data) => {
                        // Если закрыли не дожидаясь загрузки - игнорируем проставление данных
                        if (this.state.visible) {
                ...
                if (operation.selectable) {
                    ...
                    sdk.bi.getDistincts(requestParams)
                        .then((data) => {
                            // Если закрыли не дожидаясь загрузки - игнорируем проставление данных
                            if (this.state.visible) {
                                const {value} = this.state;

                                const dimensions = data.result.data.Data
                                    .map((row) => row[0])
                                    .filter((dimension) => {
                        ...
                        return (
                            <div className={b('row value-row')} key={`measure-row-${i}`}>
                                <div className={b('label')}>
                                    <span>
                                        {`${i18n('wizard', 'label_value')}${operation.manyOperands ? (' '+(i+1)) : ''}`}
                                    </span>
                                </div>
            ...
            <div>
                <div className="left-column">
                    ...
                    <div className={'options-list'}>
                        {
                            dimensions ? (leftFilteredDimensions || dimensions).map((dimension, i) => {
                                return (
                                    <div
                    ...
                    {(dimensions && !leftFilteredDimensions && dimensions.length >= VALUES_LOAD_LIMIT) && (
                        <span className="limit-message">
                            {i18n('wizard', 'label_limit-message')}
                        </span>
                    )}
                </div>
