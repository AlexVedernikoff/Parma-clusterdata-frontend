...
class DatasetTable extends React.Component {
    ...
    getVisiblePopups = () => {
        ...
        return popups
            .filter((popup) => popup.classList.contains('popup2_visible_yes'));
    };

    getDatasetPopups = (names = ['yc-select-popup', 'field-actions-popup__popup']) => {
        return names.reduce((popups, name) => [
            ...popups,
    ...
    activateDatasetRow = (e) => {
        ...
        const isClickedDropdown = parentNode.classList.contains('yc-select-control') ||
    ...
    sortAggregationColumn = (rowCurrent, rowNext) => {
        const {
            row: {
                aggregation: aggregationCurrent
            } = {}
        } = rowCurrent;
        const {
            row: {
    ...
    IndexColumn = {
        ...
        header: (
            <Icon
                className={b('header-icon-table-count')}
                data={iconTableCount}
                width="10"
            />
        ),
    ...
    TitleColumn = {
        ...
        header: (
            <div className={b('header')}>
                {i18n('dataset.dataset-editor.modify', 'column_field-name')}
            </div>
        ),
    ...
    FormulaColumn = {
        ...
        header: (
            <div className={b('header')}>
                {i18n('dataset.dataset-editor.modify', 'column_field-source')}
            </div>
        ),
        render: ({index, row}) => {
    ...
    CastColumn = {
        ...
        header: (
            <div className={b('header-cast')}>
                {i18n('dataset.dataset-editor.modify', 'column_field-cast')}
            </div>
        ),
        render: ({value, index, row}) => {
            const {types} = this.props;
    ...
    AggregationColumn = {
        ...
        header: (
            <div className={b('header')}>
                {i18n('dataset.dataset-editor.modify', 'column_field-aggregation')}
            </div>
        ),
        render: ({value, index, row}) => {
            const {types} = this.props;
    ...
    DescriptionColumn = {
        ...
        header: (
            <div className={b('header')}>
                {i18n('dataset.dataset-editor.modify', 'column_filed-description')}
            </div>
        ),
        render: ({value, index, row}) => {
            const maxTabIndex = (index + 1) * 4;
            ...
            return (
                <CustomTextInput
                    ...
                    text={value}
                    onFocus={this.onFocusTextInput}
                    onBlur={(e) => this.changeDescription({
                        row,
    ...
    MoreColumn = {
        ...
        render: ({row}) => (
            <FieldActionsPopup
                field={row}
                onClickItem={this.onClickMoreRowAction}
            />
        )
    };
