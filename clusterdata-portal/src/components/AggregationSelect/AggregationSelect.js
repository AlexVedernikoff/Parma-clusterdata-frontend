...
class AggregationSelect extends React.Component {
    ...
    get aggregationList() {
        ...
        const {
            aggregations = []
        } = types.find((type) => type.name === selectedCast) || {};
    ...
    onSelect = (data) => {
        ...
        if (this._aggregationSelectRef.current) {
            const {
                _onSwitcherClick
            } = this._aggregationSelectRef.current;
    ...
    getFieldType = () => {
        ...
        type = selectedAggregation === FieldAggregation.None ?
            'dimension' :
            'measure';
    ...
    render() {
        ...
        const aggregation = autoaggregated ?
            'auto' :
            selectedAggregation;
