...
class DNDContainer extends Component {
    ...
    render() {
        ...
        let title;
        if (this.props.title){
            title= <div className="subheader dimensions-subheader dimensions-dataset">
        ...
        return connectDropTarget(
            <div className={`dnd-container${canDrop ? ' can-drop' : ''}${isOver ? ' is-over' : ''}`}>
                {(
                    <div className="drop-place" style={{
                ...
                {items.map((item, index) => {
                    return (
                        <DNDItem
                            ...
                            listCheckAllowed={this.props.checkAllowed ? (item) => {
...
const itemTarget = {
    drop(props, monitor, component ) {
        ...
        if (id !== sourceObj.listId) {
            ...
            if (component.props.capacity && component.props.capacity <= component.state.items.length && !component.doingReplace) {
                return {
                    revert: true
                };
            }
...
export default DragDropContext(HTML5Backend)(DropTarget('ITEM', itemTarget, (connect, monitor) => ({
