    ...
    return (
        <div className={b('section')}>
            <div className={b('caption', b('margin', {bottom: 5}))}>
                <span>
                    {i18n('dataset.materialization.modify', 'section_materialization')}
                </span>
            </div>
            <RadioBox
...
function StatusPanel(props) {
    ...
    return (
        <div className={b('section', b('margin', {bottom: 22}))}>
            <div className={b('caption', b('margin', {bottom: 5}))}>
                <span>
                    {i18n('dataset.materialization.modify', 'label_status')}
                </span>
            </div>
            <div className={b('last-status', b('text-inactive'))}>
                <Status
                    isDirectDsMode={isDirectDsMode}
                    status={status}
                />
            </div>
        </div>
...
function LeftColumn(props) {
    return (
        <div className={b('column-left')}>
            <SelectionMaterializationType
                {...props}
            />
            <SelectionMaterializationSettingsType
                {...props}
            />
        </div>
    );
...
function RightColumn(props) {
    ...
    if (materializationStatus.isProcessing ||
        connectionType === 'csv' ||
        selectedDsMode !== DATASET_MODES.MATERIALIZED &&
        selectedDsMode !== DATASET_MODES.MATERIALIZED_BY_PERIOD
    ) {
...
class DataSource extends React.Component {
    ...
    onEntryClick = async (entry,closeNavModal, materializationCustomSettings) => {
        ...
        const materializationSchemaName = entry.savedId === materializationCustomSettings.connectionId ?
        ...
        const {
            response: {
                data: {
                    ...
                    error: {
                        messageError: responseErrorMessage
                    },
                } = {},
                headers: {
    ...
    fetchDataSourceData = async () => {
        ...
        try {
            ...
            const scheduleSettingsState = {
                materializationCron: materializationCron === '' ? this.state.scheduleSettings.materializationCron : materializationCron
            };

            const selectedDsMode = this.state.isMaterializationRun ? this.state.selectedDsMode :
                    (materializationCron !== '' ? DATASET_MODES.MATERIALIZED_BY_PERIOD : dsMode);
    ...
    fetchMaterializationProperties = async (sdk, dataset) => {
        ...
        try {
            const connectionId = dataset.materializationProperties.materializationConnectionId || materializationCustomSettings.connectionId;

            const materializationConnection = connectionId !== null ? await sdk.bi.getConnection({connectionId}) : null

            const materializationTableName = dataset.materializationProperties.materializationTableName || dataset.origin.table_name
            const materializationSchemaName = dataset.materializationProperties.materializationSchemaName || materializationCustomSettings.materializationSchemaName
            const materializationConnectionType = materializationConnection && materializationConnection.db_type || materializationCustomSettings.connectionType || 'Clickhouse'
            const materializationConnectionName = materializationConnection && materializationConnection.name || materializationCustomSettings.connectionName
            ...
            const modeType = dataset.materializationProperties.materializationConnectionId === null ?
    ...
    updateDataset = async () => {
        ...
        const isDefaultMode = materializationCustomSettings.modeType === MaterializationSettings.DATASET_MATERIALIZED_MODES.DEFAULT
    ...
    clickSaveBtn = async () => {
        ...
        const {
            ...
            materializationStatus: {
                isProcessing: isProcessingMaterialization
            } = {},
            materializationCustomSettings
        } = this.state;
            ...
            switch (value) {
                case 'open-connection': {
                    const {
                        dataset: {
                            origin: {
                                table_connection_id: connectionId = ''
                            } = {}
                        } = {}
                    } = this.state;
    ...
    renderContent = () => {
        const {
            ...
            dataset: {
                ...
                origin,
                origin: {
                    table_db_name: tableDbNameOrigin
                } = {}
            } = {},
            materializationStatus: {
                isProcessing: isProcessingMaterialization
        ...
        const disableDeleteMaterialization = isDirectDsMode ||
            isMaterializationDeleting ||
            isProcessingMaterialization ||
            DISABLE_TYPES.includes(connectionType);
