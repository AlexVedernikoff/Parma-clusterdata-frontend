...
function MaterializationCustomSettings(props) {
    ...
    const handleChange = (name, value) => {
        if ((name === 'materializationThreadCount' ||
    ...
    return (
        <div className={b('params')}>
            <div className={b('dataset-panel')}>
                <Icon className={b('connector-ic')} data={Utils.getConnectorIcon({type: materializationCustomSettings.connectionType})} width="32"/>
                <span className={b('label-connection-type')}>{getConnectorsMap()[materializationCustomSettings.connectionType]}</span>
                <span className={b('label-connection-name')}>{materializationCustomSettings.connectionName}</span>
                <SelectConnection sdk={sdk} connectionId={materializationCustomSettings.connectionId} onEntryClick={entryClick}/>
            </div>
            <div className="row">
