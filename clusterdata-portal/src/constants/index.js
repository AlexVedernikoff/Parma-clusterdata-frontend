...
export const getFakeEntry = entryType => {
    ...
    return {
        fake: true,
        key: window.DL.user.login ? `/Users/${login}/${i18n('connections.form', `section_creation-${entryType}`)}` : `/${i18n('connections.form', `section_creation-${entryType}`)}`,
        entryId: null
    };
};
...
const SCATTER_VISUALIZATION = {
    ...
    checkAllowedSort: (item, visualization) => {
        ...
        const selectedItems = visualization.placeholders.slice(0, 2)
            .reduce((a, b) => a.concat(b.items), [])
            .filter((selectedItem) => selectedItem.type === 'DIMENSION');
