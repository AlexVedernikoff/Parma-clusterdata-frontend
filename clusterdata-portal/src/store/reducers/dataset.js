...
function checkFetchingPreview({updatePreview, updates}) {
    return !updatePreview && updates && updates.length &&
    ...
    return async (dispatch, getState, {toaster}) => {
        ...
        const {
            dataset: {
                ...
                history: {
                    amountPreviewRows: amountHistoryRows
                } = {},
                validation: {
...
export function initialFetchDataset({datasetId, datasetErrorDialogRef}) {
    return async (dispatch, getState, {sdk}) => {
        try {
            ...
            if (!previewEnabled) {
                dispatch(toggleVisibilityPreview({
                    isVisible: false
                }));
            }
...
const dispatchFetchPreviewDataset = async ({
    ...
    try {
        ...
        const resultSchemasWithSource = resultSchema.filter(rs =>
            rs.source !== ''
            && rs.formula === ''
