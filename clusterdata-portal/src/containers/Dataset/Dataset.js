...
class Dataset extends React.Component {
        ...
        try {
            ...
            switch (item) {
                ...
                case REPLACE_SOURCE_MODE_ID: {
                    ...
                    window.open(
                        `/datasets/${datasetId}/source?id=${connectionId}`,
                        '_blank'
                    );
    ...
    renderErrorContent() {
        ...
        const {
            ...
            description,
            action
        } = this.getErrorMessageByCode({status, data});
        ...
        return (
            <React.Fragment>
                ...
                />
                <EntryDialogues
                    ref={this._askAccessRightsDialogRef}
                    sdk={sdk}
                />
            </React.Fragment>
        );
        ...
        if (isLoading) {
            return (
                <div className={b('loader')}>
                    <ContainerLoader
                        text={i18n('dataset.dataset-editor.modify', 'label_loading-dataset')}
                    />
                </div>
            );
        }
                        ...
                        (
                            <Button
                                ...
                                text={
                                    isProcessingDataset ?
                                        null :
                                        i18n('dataset.dataset-editor.modify', 'button_save')
                                }
                                onClick={() => saveDataset({
                                    datasetErrorDialogRef: this.datasetErrorDialogRef
                                })}
