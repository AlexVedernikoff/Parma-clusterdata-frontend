...
const AddButton = ({text, onClick}) => (
    <Button
        ...
        tone="default"
        size="s"
        iconLeft={<ButtonIcon><Icon data={iconPlus} width="16"/></ButtonIcon>}
        text={text}
        onClick={onClick}
    />
);
                        ...
                        <div className={b('param')} key={'param' + keyIndex}>
                            <div className={b('column')}>
                                <div className={b('input')}>
                                    <Input
                                        text={key}
                                        onChange={text => setEntries(entries.map((pair, index) =>
                            ...
                            <div className={b('column')}>
                                {values.map((value, valueIndex) => (
                                    <div className={b('input')} key={'value' + valueIndex}>
                                        ...
                                        />
                                        {values.length > 1 &&
                                            <Remove onClick={() => setEntries(entries.map((pair, index) =>
                                                index === keyIndex ?
                                ...
                                <AddButton
                                    ...
                                    onClick={() => setEntries(entries.map((pair, index) =>
