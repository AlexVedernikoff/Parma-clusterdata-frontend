...
const getDefaultRangeType = (range) => {
    ...
    for (const rangeType in defaultRanges) {
        ...
        if (range.from === defaultRange.from &&
...
const singleItems = [
    {val: FILTER_TYPES.NoDefined, get text() { return i18n('dash.control-dialog.edit', 'value_undefined'); }},
    {val: FILTER_TYPES.AcceptableFrom, get text() { return i18n('dash.control-dialog.edit', 'value_date-accepted-start'); }, sourceType: CONTROL_SOURCE_TYPE.MANUAL},
    {val: FILTER_TYPES.AcceptableTo, get text() { return i18n('dash.control-dialog.edit', 'value_date-accepted-end'); }, sourceType: CONTROL_SOURCE_TYPE.MANUAL},
    {val: FILTER_TYPES.Date, get text() { return i18n('dash.control-dialog.edit', 'value_date-manual'); }},
    {val: FILTER_TYPES.Relative, get text() { return i18n('dash.control-dialog.edit', 'value_date-relative'); }}
];

const rangeItems = [
    {val: FILTER_TYPES.NoDefined, get text() { return i18n('dash.control-dialog.edit', 'value_undefined'); }},
    {val: FILTER_TYPES.AcceptableFullInterval, get text() { return i18n('dash.control-dialog.edit', 'value_date-accepted-all'); }, sourceType: CONTROL_SOURCE_TYPE.MANUAL},
    {val: FILTER_TYPES.Date, get text() { return i18n('dash.control-dialog.edit', 'value_date-manual'); }},
    {val: FILTER_TYPES.Relative, get text() { return i18n('dash.control-dialog.edit', 'value_date-relative'); }},
    {val: FILTER_TYPES.DefaultRanges, get text() { return i18n('dash.control-dialog.edit', 'value_date-default-ranges'); }}
];
...
class Default extends React.PureComponent {
    ...
    get initDefaultValues() { return {type: '', value: {from: '', to: ''}}; }
    ...
    isValidNumber(text) { return /^(-?\d*)$/.test(text); }
    ...
    renderRange() {
        ...
        return rangeItems.map(item => !item.sourceType || item.sourceType === sourceType ?
    ...
    renderSingle() {
        ...
        return singleItems.map(item => !item.sourceType || item.sourceType === sourceType ?
            <React.Fragment key={item.val}>
                <div className={b('radiobox-place')}>
                    <RadioBox
                        theme="normal"
