...
function Param({name, value, onRemove}) {
    if (Array.isArray(value)) {
        return (
            <React.Fragment>
                {value.map((val) => <Param key={name + val} name={name} value={val} onRemove={onRemove}/>)}
            </React.Fragment>
        );
    }
    return (
...
function wrapToArray(value) { return Array.isArray(value) ? value : [value]; }
...
class Widget extends React.PureComponent {
    ...
    static defaultProps = {
        data: [{
            data: {},
            description: '',
            get title() { return i18n('dash.widget-dialog.edit', 'value_title-default', {index: 1}); }
        }]
    };
    ...
    get isEdit() { return Boolean(this.props.id); }

    onApply = () => {
        const {data} = this.state;
        const tabIndex = data.findIndex(({data: {uuid}}) => !uuid);
        if (tabIndex === -1) {
            this.props.setItemData({data: data.map(({title, ...rest}, index) =>
                ({
    ...
    _onAddWidget = ({entryId, name, params = {}}) => {
        ...
        if (isManualTitle) {
            this.setState({data: update(data, {[tabIndex]: {
                data: {
                    uuid: {$set: entryId},
    ...
    addParam = () => {
        ...
        this.setState({data: update(data, {
            [tabIndex]: {data: {params: {$set: mergeWith({}, params, {[paramKey]: paramValue}, (objValue, srcValue) => {
                if (objValue) {
    ...
    removeParam = (paramKey, paramValue) => {
        ...
        this.setState({data: update(data, {
            [tabIndex]: {data: {params: {$set: Object.entries(params).reduce((result, [key, value]) => {
                if (key === paramKey) {
    ...
    renderParams() {
        ...
        const {data: {params = {}}} = data[tabIndex];
            ...
            return (
                <React.Fragment>
                    <Line caption={i18n('dash.widget-dialog.edit', 'field_params')}>
                        ...
                        >
                            <ButtonIcon><Icon data={iconPlus} width="16"/></ButtonIcon>
                    ...
                    <div className={b('params', {empty: !Object.keys(params).length})}>
                        {Object.keys(params).length ?
                            Object.entries(params).map(([key, value]) =>
                                <Param
                                    key={key + value} name={key} value={value} onRemove={this.removeParam}
                                />
                            ) :
                        ...
                        <div className={b('main')}>
                            ...
                            <div className={b('content')}>
                                ...
                                <TextArea
                                    ...
                                    onChange={value => this.setState({data: update(data, {
