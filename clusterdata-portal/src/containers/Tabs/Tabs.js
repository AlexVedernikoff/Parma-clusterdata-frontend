...
function Tabs(props) {
    ...
    const hideShowTabsElements = () => {
        ...
        const getVisibleTabsWidth = () => visibleTabElems.reduce((acc, cur) => acc + cur.offsetWidth + tabMargin, 0) + popupBtnWidth + delta;
        const getVisibleTabsWidthWithFirstHidden = () => [...visibleTabs, hiddenTabs[0]].reduce((acc, cur) => {
            return acc + cur.htmlWidth + tabMargin;
        }, 0) + popupBtnWidth + delta;
