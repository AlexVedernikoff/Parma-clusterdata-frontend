...
import { NavigationBreadcrumbsInterface } from './navigation-breadcrumbs.interface'
import { NavigationPlace } from '../navigation-place.enum'

export class NavigationBreadcrumbsModel implements NavigationBreadcrumbsInterface {
  constructor(value?: NavigationBreadcrumbsInterface) {
    const { size, path, place, onClick, linkWrapper, getPlaceParameters } = value || {}
    this.size = size || NavigationBreadcrumbsModel.size
    this.path = path || NavigationBreadcrumbsModel.path
    this.place = place || NavigationBreadcrumbsModel.place
