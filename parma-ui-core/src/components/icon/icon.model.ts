 ...
 * Модель пиктограммы
 */
export class IconModel implements IconInterface {
  constructor({ data, width, height, className, prefix, onClick, fill, stroke }: IconInterface) {
    this.data = data
    this.width = width
    this.height = height
    ...
    this.fill = fill || IconModel.fill
    this.stroke = stroke || IconModel.stroke
  }
  ...
  private viewBoxValue(index: number): number {
    return UnitsHelper.isNullOrUndefined(this.data.viewBox)
      ? 0
      : UnitsHelper.convertToNumber(IconModel.splitViewBox(this.data.viewBox)[index])
  }
