...
import { Button, Tooltip, Link } from 'lego-on-react';
...
function ControlIcon(props) {
  ...
  return <Icon data={iconQuestionMark} width={16} height={16} onClick={onClick} />;
}
...
function TooltipTitle(props) {
  ...
  return <h3 className={b('popup-title')}>{title}</h3>;
}

function TooltipContent(props) {
  const { tone, theme, content, htmlContent } = props;
  ...
  if (content) {
    return <div className={b('popup-content', { tone })}>{content}</div>;
  }
}
  ...
  return (
    <div className={b('popup-links')}>
      {links.map((link, index) => {
        const { text, url, target = '_blank', onClick } = link;
...
function TooltipButton(props) {
  const { size, tooltipButton } = props;
...
export default class HelpTooltip extends Component {
  ...
  onMouseOut = () => {
    const { autoclosable, delayClosing } = this.props;
  ...
  render() {
    ...
    return (
      ...
      >
        <ControlIcon onClick={this.toggleTooltip} />
        <Tooltip
        ...
        >
          {children ? (
            ...
            <React.Fragment>
              ...
              />
              <TooltipLinks links={links} />
              <TooltipButton size={size} tooltipButton={tooltipButton} />
            </React.Fragment>
          )}
        </Tooltip>
      </div>
