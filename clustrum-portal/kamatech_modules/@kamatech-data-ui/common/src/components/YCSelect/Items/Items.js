...
export default class ItemsWrapper extends React.PureComponent {
  ...
  _itemRenderer = ({ index, style }) => {
    const { items, innerValue, showMoreLinkVisible } = this.props;
    const { isFetchingItemsByLink } = this.state;
  ...
  _renderItemsWithoutVirtualized() {
    const { items, innerValue, isItemsGrouped } = this.props;
    ...
    if (isItemsGrouped) {
      return items.map(item => {
        ...
        const groupTitleNode = (
          <div key={groupTitle} className={b('group-title')}>
            {groupTitle}
          </div>
        );
  ...
  render() {
    ...
    if (!isDynamic && items.length < virtualizeThreshold) {
      ...
      return (
        <div ref={this.ref} className={b(wrapperMods)}>
