...
export default function YearSwitcher(props) {
  ...
  return (
    <div className={b('year-switcher')} onClick={onSwitcherClick}>
      <div className={b('year-switcher-arrow', { left: true })} data-dir="prev">
        <Icon data={iconArrow} />
      </div>
      <div className={b('year-switcher-val')}>{year}</div>
      <div className={b('year-switcher-arrow', { right: true })} data-dir="next">
        <Icon data={iconArrow} />
      </div>
    </div>
  );
