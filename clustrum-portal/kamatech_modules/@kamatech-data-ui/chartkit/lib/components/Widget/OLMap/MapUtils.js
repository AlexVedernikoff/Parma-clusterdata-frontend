...
class MapUtils {
  ...
  static calculateSectors(data, radius, iconCache) {
    ...
    let svg = new Image();
    svg.src =
      'data:image/svg+xml,<svg style="width: 40px; height: 40px" xmlns="http://www.w3.org/2000/svg">' +
      escape(newSVG.innerHTML) +
      '</svg>';
  ...
  static splitStringBySouthands(value) {
    return isFinite(value) ? NumberValue.toLocaleString(value) : value;
  }
    ...
    features
      ...
      .forEach((feature, index) => {
        gradient[new FeatureColor(feature).featureColorKey()] =
          features.length > 0 ? MapUtils.hexToRgb(rainbow.colourAt(index)) : '#FF445D';
      });
