  ...
  return (
    <div className={b('cell')}>
      {items.map((item, index) => (
        <div className={b('breadcrumb')} key={generateKey(item.name)}>
...
export default function StatSourceView({ index, source }) {
  ...
  return (
    <div className={b('row', { border_bottom: true })} key={`${index}_${source.url}`}>
      ...
      <div className={b('column', { left: true })}>{_source.name}</div>
      ...
      <div className={b('column', { right: true })}>{_dimensions}</div>
    </div>
  );
}
